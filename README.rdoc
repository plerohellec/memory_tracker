...
== Tips
* Ruby processes rarely leak memory. Instead they allocate large amounts of memory and never release it. This memory is not lost, it's still available to the process that allocated it but it's unavailable for all other processes in the system.
